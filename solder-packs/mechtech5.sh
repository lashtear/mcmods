...
if [ "$1" = "reset" ]; then
    ( cd $HOME/git/mechtech5-conf.git; git tag -d mechtech5-$MPVERSION )
    ( cd $HOME/git/mechanipus-resource-pack.git; git tag -d mechtech5-$MPVERSION )
    exit 0
fi
...
( cd $HOME/git/mechtech5-conf.git; git tag mechtech5-$MPVERSION )
if [ $? -ne 0 ]; then
    echo tag failed >&2
    exit 1
fi
gitfile "$HOME/git/mechtech5-conf.git" magitech-config "$MPVERSION"
( cd $HOME/git/mechanipus-resource-pack.git; git tag mechtech5-$MPVERSION )
if [ $? -ne 0 ]; then
    echo tag failed >&2
    exit 1
fi
stagegitfile "$HOME/git/mechanipus-resource-pack.git" mechanipus "$MPVERSION"
solderfile --staged --client mechanipus.zip mechanipus-resource-pack-mechtech5 "$MPVERSION" resourcepacks mechanipus.zip
